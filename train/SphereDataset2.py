...
class SphereDataSetCodes(Dataset):
    
    def __init__(self, prex, train = True, img_dir='/data1/home/csmuli/360_dataset/imp/mse/'):
        self.img_path = '{}/{}/train'.format(img_dir,prex) if train else '{}/{}/test'.format(img_dir,prex)
        if not train:
            self.img_list = ['{}.png'.format(idx) for idx in range(200)]
        else:
            f = open('/data1/home/csmuli/360_dataset/train.txt')
            nlines = len(f.readlines())
            f.close()
            self.img_list = ['{}.png'.format(idx) for idx in range(nlines)]
    ...
    def __getitem__(self, idx):
        ...
        img = torch.from_numpy(img/5).type(torch.float32).unsqueeze(0)
        img = img.contiguous()
        return img

def load_train_test_distribute_for_codes(world_size,rank, batch_size, test_batch_size, prex, shuffle=True):
    kwargs = {'num_workers': 4, 'pin_memory': True}
    train_data = SphereDataSetCodes(prex,True)
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_data,num_replicas=world_size,rank=rank,shuffle=shuffle)
    train_loader = torch.utils.data.DataLoader(
        train_data, sampler=train_sampler,
        batch_size=batch_size, shuffle=False, **kwargs)
    test_loader = torch.utils.data.DataLoader(
        SphereDataSetCodes(prex,False),
        batch_size=test_batch_size, shuffle=False, **kwargs)
    return train_loader, test_loader

if __name__ == '__main__':
    train_loader, test_loader = load_train_test_distribute_for_codes(4,0,3,3,prex='low_imp_ent_5_1000_61_61_0_1_v0_best_0')
