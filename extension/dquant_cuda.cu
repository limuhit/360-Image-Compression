...
void dquant_opt::reshape(int num, int channel, int height, int width){
    ...
    //zero_ = at::zeros({nchannel_},at::kFloat).to(torch::Device(torch::kCUDA, device_));
}
...
__global__ void dquant_cal_weight_kernel(const int nthreads, const scalar_t* const input, scalar_t * const output, const int level) {
...
template <typename scalar_t>
__global__ void dquant_forward_kernel(const int nthreads, const scalar_t* const input, const scalar_t * const weight, const scalar_t* const mask,
...
std::vector<at::Tensor>  dquant_opt::forward_cuda(at::Tensor  bottom_data, at::Tensor mask, at::Tensor weight_old) 
{
	...
	AT_DISPATCH_FLOATING_TYPES(
		bottom_data.scalar_type(), "dquant_forward_cuda", 
			([&] {
                    count = nchannel_;
                    dquant_cal_weight_kernel << <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0 , stream_ >> >
                    ...
                    dquant_forward_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
                        (count, bottom_data.data_ptr<scalar_t>(), weight_.data_ptr<scalar_t>(), mask.data_ptr<scalar_t>(),
                        top_data_[0].data_ptr<scalar_t>(),  width_*height_, channel_, bin_num_);
