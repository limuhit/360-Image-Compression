...
void quant_opt::update_weight(at::Tensor weight, at::Tensor ncount){    
...
std::vector<at::Tensor>  quant_opt::quant_forward_cuda(at::Tensor  bottom_data, at::Tensor weight_old, at::Tensor ncount,bool train) 
{
	...
	AT_DISPATCH_FLOATING_TYPES(
		bottom_data.scalar_type(), "quant_forward_cuda", 
			([&] {
                    ...
                    cudaMemset(count_data_.data_ptr<float>(), float(0.0),  count*sizeof(float));
