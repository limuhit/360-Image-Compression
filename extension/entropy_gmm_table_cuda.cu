...
__global__ void entropy_gmm_table_batch_forward_kernel(const int nthreads, const scalar_t* const data, scalar_t * const output, 
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
        ...
        if(pt==0){
            ...
            scalar_t v = pt - 1 - bias + 0.5, ps=0, f;
            for(int i = 0; i<w;i++){
                f = 0.5+0.5*erf(s2*(v-data[2*stride+pn*w+i])/data[stride+pn*w+i]);
                //if(f<=0 || f!=f) printf("f %d %f\n",index,f);
                //if(data[pn*w+i]<=0 || data[pn*w+i]!=data[pn*w+i]) printf("w %d %f\n",index,data[pn*w+i]);
                ps = ps + data[pn*w+i]*f;
            } 
            output[index] = static_cast<int>(total*ps+0.5);
            //if(output[index]<0) printf("v %d %f %f\n", index, ps, output[index]);
        }
    }
}
